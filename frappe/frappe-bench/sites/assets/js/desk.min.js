...
frappe.socket.doc_subscribe(frm.doctype,frm.docname);});$(document).on("form_refresh",function(e,frm){frappe.socket.doc_open(frm.doctype,frm.docname);});$(document).on('form-unload',function(e,frm){frappe.socket.doc_close(frm.doctype,frm.docname);});window.onbeforeunload=function(){if(cur_frm&&cur_frm.doc){frappe.socket.doc_close(cur_frm.doctype,cur_frm.docname);}}},get_host:function(){var host=frappe.urllib.get_base_url();if(frappe.boot.dev_server){parts=host.split(":");if(parts.length>2){host=parts[0]+":"+parts[1];}
host=host+":3000";}
...
prev=fields[i];}
return false;},focus_on_next_field:function(start_idx,fields){for(var i=start_idx+1,len=fields.length;i<len;i++){var field=fields[i];if(field.disp_status==="Write"){if(field.df.fieldtype==="Table"){if(!(field.grid.grid_rows&&field.grid.grid_rows.length)){field.grid.add_new_row();}
field.grid.grid_rows[0].show_form();}else if(!in_list(frappe.model.no_value_type,field.df.fieldtype)){this.set_focus(field);break;}}}},set_focus:function(field){if(field.df.fieldtype=="Table"){if(!field.grid.grid_rows.length){field.grid.add_new_row(1);}else{field.grid.grid_rows[0].toggle_view(true);}}
...
this.$value.toggle(true).find(".attached-file").html(filename||this.value).attr("href",dataurl||this.value);}else{this.$input.toggle(true);this.$value.toggle(false);}},get_value:function(){if(this.frm){return this.value;}else{return this.fileobj?(this.fileobj.filename+","+this.dataurl):null;}},on_upload_complete:function(attachment){if(this.frm){this.parse_validate_and_set_in_model(attachment.file_url);this.refresh();this.frm.attachments.update_attachment(attachment);}else{this.value=this.get_value();this.refresh();}},});frappe.ui.form.ControlAttachImage=frappe.ui.form.ControlAttach.extend({make_input:function(){var me=this;this._super();this.img_wrapper=$('<div style="margin: 7px 0px;">\
   <div class="missing-image attach-missing-image"><i class="octicon octicon-circle-slash"></i></div></div>').prependTo(this.input_area);this.img=$("<img class='img-responsive attach-image-display'>").appendTo(this.img_wrapper).toggle(false);this.img_wrapper.find(".missing-image").on("click",function(){me.$input.click();});this.img.on("click",function(){me.$input.click();});this.$wrapper.on("refresh",function(){me.set_image();});this.set_image();},set_image:function(){if(this.get_value()){$(this.input_area).find(".missing-image").toggle(false);this.img.attr("src",this.dataurl?this.dataurl:this.value).toggle(true);}else{$(this.input_area).find(".missing-image").toggle(true);this.img.toggle(false);}}});frappe.ui.form.ControlSelect=frappe.ui.form.ControlData.extend({html_element:"select",make_input:function(){var me=this;this._super();this.set_options();},set_input:function(value){this.set_options(value||"");this._super(value);var input_value=this.$input.val();if(this.doctype&&this.docname){var model_value=frappe.model.get_value(this.doctype,this.docname,this.df.fieldname);if(model_value==null&&(input_value||"")!=(model_value||"")){this.set_model_value(input_value);}else{this.last_value=value;}}else{if(value!==input_value){this.set_value(input_value);}}},set_options:function(value){var options=this.df.options||[];if(typeof this.df.options==="string"){options=this.df.options.split("\n");}
...
frappe.ui.notifications.update_notifications=function(){frappe.ui.notifications.total=0;var doctypes=keys(frappe.boot.notification_info.open_count_doctype).sort();var modules=keys(frappe.boot.notification_info.open_count_module).sort();frappe.ui.notifications.navbar_notification=$("#navbar-notification").empty();frappe.ui.notifications.sidebar_notification=$("#sidebar-notification").empty();frappe.ui.notifications.add_notification("Comment");frappe.ui.notifications.add_notification("ToDo");frappe.ui.notifications.add_notification("Event");if(frappe.ui.notifications.total){var divider='<li class="divider"></li>';frappe.ui.notifications.navbar_notification.append($(divider));frappe.ui.notifications.sidebar_notification.append($(divider));}
$.each(doctypes,function(i,doctype){if(!in_list(["ToDo","Comment","Event"],doctype)){frappe.ui.notifications.add_notification(doctype);}});$("#navbar-notification a, #sidebar-notification a").on("click",function(){var doctype=$(this).attr("data-doctype");var config=frappe.ui.notifications.config[doctype]||{};if(config.route){frappe.set_route(config.route);}else{frappe.views.show_open_count_list(this);}});$(".navbar-new-comments").html(frappe.ui.notifications.total).toggleClass("navbar-new-comments-true",frappe.ui.notifications.total?true:false);}
frappe.ui.notifications.add_notification=function(doctype){var count=frappe.boot.notification_info.open_count_doctype[doctype];if(count){var config=frappe.ui.notifications.config[doctype]||{};var label=config.label||doctype;var notification_row=repl('<li><a class="badge-hover" data-doctype="%(data_doctype)s">\
   <span class="badge pull-right">\
    %(count)s</span> \
   %(label)s </a></li>',{label:__(label),icon:frappe.boot.doctype_icons[doctype],count:count,data_doctype:doctype});frappe.ui.notifications.navbar_notification.append($(notification_row));frappe.ui.notifications.sidebar_notification.append($(notification_row));frappe.ui.notifications.total+=count;}}
frappe.ui.notifications.config={"ToDo":{label:__("To Do")},"Comment":{label:__("Messages"),route:"messages"},"Event":{label:__("Calendar"),route:"Calendar/Event"}};frappe.views.show_open_count_list=function(element){var doctype=$(element).attr("data-doctype");var condition=frappe.boot.notification_info.conditions[doctype];if(condition&&$.isPlainObject(condition)){frappe.route_options=condition;}
...
this.prepare();this.dialog.show();},get_fields:function(){var cc_fields=this.get_cc_fields();var fields_before_cc=[{fieldtype:"Section Break"},{label:__("To"),fieldtype:"Data",reqd:0,fieldname:"recipients"},{fieldtype:"Section Break",collapsible:1,label:"CC & Standard Reply"},{label:__("CC"),fieldtype:"Data",fieldname:"cc"},];var fields_after_cc=[{label:__("Standard Reply"),fieldtype:"Link",options:"Standard Reply",fieldname:"standard_reply"},{fieldtype:"Section Break"},{label:__("Subject"),fieldtype:"Data",reqd:1,fieldname:"subject"},{fieldtype:"Section Break"},{label:__("Message"),fieldtype:"Text Editor",reqd:1,fieldname:"content"},{fieldtype:"Section Break"},{fieldtype:"Column Break"},{label:__("Send As Email"),fieldtype:"Check",fieldname:"send_email"},{label:__("Send me a copy"),fieldtype:"Check",fieldname:"send_me_a_copy"},{label:__("Communication Medium"),fieldtype:"Select",options:["Phone","Chat","Email","SMS","Visit","Other"],fieldname:"communication_medium"},{label:__("Sent or Received"),fieldtype:"Select",options:["Received","Sent"],fieldname:"sent_or_received"},{label:__("Attach Document Print"),fieldtype:"Check",fieldname:"attach_document_print"},{label:__("Select Print Format"),fieldtype:"Select",fieldname:"select_print_format"},{fieldtype:"Column Break"},{label:__("Select Attachments"),fieldtype:"HTML",fieldname:"select_attachments"}];return fields_before_cc.concat(cc_fields).concat(fields_after_cc);},get_cc_fields:function(){if(!(this.frm&&this.frm.doc)){return[];}
var cc=[[this.frm.doc.owner,1]];var starred_by=frappe.ui.get_starred_by(this.frm.doc);if(starred_by){for(var i=0,l=starred_by.length;i<l;i++){cc.push([starred_by[i],1]);}}
var assignments=this.frm.get_docinfo().assignments;if(assignments){for(var i=0,l=assignments.length;i<l;i++){cc.push([assignments[i].owner,1]);}}
var comments=this.frm.get_docinfo().comments;if(comments){for(var i=0,l=comments.length;i<l;i++){cc.push([comments[i].comment_by,0]);}}
var added=[];var cc_fields=[];for(var i=0,l=cc.length;i<l;i++){var email=cc[i][0];var default_value=cc[i][1];if(!email||added.indexOf(email)!==-1||email.indexOf("@")===-1){continue;}
added.push(email);email=frappe.user.get_formatted_email(email);cc_fields.push({"label":frappe.utils.escape_html(email),"fieldtype":"Check","fieldname":email,"is_cc_checkbox":1,"default":default_value});}
return cc_fields;},prepare:function(){this.setup_subject_and_recipients();this.setup_print();this.setup_attach();this.setup_email();this.setup_autosuggest();this.setup_last_edited_communication();this.setup_standard_reply();$(this.dialog.fields_dict.recipients.input).val(this.recipients||"").change();$(this.dialog.fields_dict.subject.input).val(this.subject||"").change();this.setup_earlier_reply();},setup_subject_and_recipients:function(){this.subject=this.subject||"";if(!this.recipients&&this.last_email){this.recipients=this.last_email.comment_by;}
...
frappe.utils.if_notify_permitted();frappe.realtime.on("csrf_generated",function(data){if(data.csrf_token&&data.sid===frappe.get_cookie("sid")){frappe.csrf_token=data.csrf_token;}});},load_bootinfo:function(){if(frappe.boot){frappe.modules=frappe.boot.modules;frappe.model.sync(frappe.boot.docs);$.extend(frappe._messages,frappe.boot.__messages);this.check_metadata_cache_status();this.set_globals();this.sync_pages();moment.locale(frappe.boot.lang);moment.user_utc_offset=moment().utcOffset();if(frappe.boot.timezone_info){moment.tz.add(frappe.boot.timezone_info);}
...
return module;};;frappe.templates["error_object"] = '{% if (Object.prototype.toString.call(x) === "[object Object]") { %} <table class="table"> {% for (var key in x) { %} <tr> <td><code>{{ key }}</code></td> <td>{{ x[key] }}</td> </tr> {% } %} </table> {% } else { %} {{ x }} {% } %} ';
